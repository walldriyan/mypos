// src/components/debtors/AddSalePaymentForm.tsx
"use client";
...
import { addSalePaymentAction } from "@/lib/actions/debtor.actions";
...
interface AddSalePaymentFormProps {
  transaction: DebtorTransaction;
  onSuccess: () => void;
}

export function AddSalePaymentForm({ transaction, onSuccess }: AddSalePaymentFormProps) {
  const { toast } = useToast();
  const [isSubmitting, setIsSubmitting] = useState(false);
  
  const balance = transaction.finalTotal - transaction.totalPaid;

  const form = useForm<DebtorPaymentFormValues>({
    resolver: zodResolver(debtorPaymentSchema),
    defaultValues: {
  ...
  async function onSubmit(data: DebtorPaymentFormValues) {
    ...
    const result = await addSalePaymentAction(data);
